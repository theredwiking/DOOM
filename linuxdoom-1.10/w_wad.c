...
void W_AddFile(char *filename) {
  ...
  if (strcmpi(filename + strlen(filename) - 3, "wad")) {
    ...
    if (read(handle, &header, sizeof(header)) < 0) {
      fprintf(stderr, "Read(header) failed in W_AddFile\n");
      fflush(stderr);
    }
    ...
    if (read(handle, fileinfo, length) < 0) {
      fprintf(stderr, "Failed to read(fileinfo) in W_AddFile\n");
      fflush(stderr);
    }
...
void W_Reload(void) {
  ...
  if (read(handle, &header, sizeof(header)) < 0) {
    fprintf(stderr, "Failed to read(header) in W_Reload\n");
    fflush(stderr);
  }
  ...
  lseek(handle, header.infotableofs, SEEK_SET);
  if (read(handle, fileinfo, length) < 0 ) {
		fprintf(stderr, "Failed to read(fileinfo) in W_Reload\n");
		fflush(stderr);
	}
